...
- Sometimes the bookmark star icon may disappear and/or stop working properly after clicking somewhere that you're not supposed to. Meaning a location that's not a road/unlabeled land (i.e. gray square building, name of neighbourhood, city, country, hospital, supermarket, etc. Basically anything that turns the Google Maps marker red). Reloading Google Maps with its base (maps.google.com) address resets the content script and it works again. So, if you happen to break something, go to maps.google.com and it will probably start working again;

- This extension was made and tested on a 2560x1440 monitor, therefore overflows and other visual glitches are to be expected in different resolutions. Even in the same resolution as mine as well;
...
- Whenever you remove/add a bookmark through Google Maps/Street View, the extension page is meant to reload. However, if you have multiple instances of the extension page open, only one of the will reload and update, while the other ones remain unchanged until you refresh the page. It seems to be the case that it is always the tab that's the leftmost tab.
